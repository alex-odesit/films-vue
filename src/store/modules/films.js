...
export default {
  mutations: {
    ...
    async changeFilm(state, array) {
      ...
      await DB.sendData(
        `films/${array[2]}s/${array[0]}`,
        state.currentFilms[array[0]]
      ).then(() => {
    ...
    async addNewFilm(state, array) {
      ...
      if (array[1] === "currentFilm") {
        if (state.currentFilms.length !== 0) {
          let newId = String(
            Number(state.currentFilms[state.currentFilms.length - 1].id) + 1
          );
          newFilm.ru.id = newId;
          newFilm.uk.id = newId;
        } else{
          let newId = String(state.currentFilms.length);
          newFilm.ru.id = newId;
          newFilm.uk.id = newId;
        }
        ...
        if (state.futureFilms.length !== 0) {
          let newId = String(
            state.futureFilms[state.futureFilms.length - 1].id + 1
          );
          newFilm.ru.id = newId;
          newFilm.uk.id = newId;
        } else {
          let newId = String(state.futureFilms.length);
          newFilm.ru.id = newId;
          newFilm.uk.id = newId;
        }
